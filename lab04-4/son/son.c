...
void *listen_to_neighbor(void *arg) {
    ...
    while (1)
    {
        ...
        if (flag == -1) {
            ...
            //向sip发送报文，有一个节点已经断开
            //我们需要填充心得type和哪个节点崩了
            other_node_sip_pkt.header.type=POINT_KILLED;
            other_node_sip_pkt.header.src_nodeID=nt[nbr_index].nodeID;
            other_node_sip_pkt.header.dest_nodeID=topology_getMyNodeID();
            printf("son :send pkt to sip :one nbr %d killed\n",nt[nbr_index].nodeID);
...
void waitSIP() {
    ...
    while(1)
    {
        ...
        {
            printf("son don't send nextnode -1 \n");
        }
        else//单点发送
        {
            //获得邻居数
            ...
            for(int nbr_index=0;nbr_index<nbrnum;nbr_index++)
            {
                if(nt[nbr_index].nodeID==nextNode)
                {
                    int sendflag=sendpkt(&sip2son_pkt,nt[nbr_index].conn);
                    assert(sendflag==1);
                    break;
...
void son_stop()
{
    ...
    //循环删除所有的sockfd，本来应该调用函数，但是比较短就在这写了。
    for(int nbr_index=0;nbr_index<nbrnum;nbr_index++)
    {
        close(nt[nbr_index].conn);
    }
