...
int stcp_client_connect(int sockfd, int nodeID, unsigned int server_port)
{
    ...
    switch (tcb->state) {
        case CLOSED:
            ...
            //发送报文
            sip_sendseg(sip_conn,tcb->server_nodeID, &syn);
    ...
    while(1) {
        switch (tcb->state)
        {
            ...
            case CONNECTED:
                ...
                //发送FIN
                sip_sendseg(sip_conn,tcb->server_nodeID, &fin);
...
void sendBuf_timeout(client_tcb_t* clienttcb)
{
    ...
    for(i=0;i<clienttcb->unAck_segNum;i++)
    {
        ...
        sip_sendseg(sip_conn,clienttcb->server_nodeID, &(bufPtr->seg));
        struct timeval currentTime;
        gettimeofday(&currentTime,NULL);
